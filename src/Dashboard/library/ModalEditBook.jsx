...
const ModalEditBook = ({ item, open, handleOpen, getBookList }) => {
  ...
  useEffect(() => {
    ...
    setTitle(item.title);
    setIsbn_no(item.isbn_no);
    setRack_no(item.rack_no);
    setBook_no(item.book_no);
  ...
  const onSubmitClick = () => {
    ...
    fetch(baseurl + "/api/library/" + item.book_no, {
      method: "PUT",
      headers: {
